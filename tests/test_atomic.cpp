...
namespace atomic_stack_tests {
    ...
    void check_stack_allocations(const std::vector<StackLinkType>& allocations, const Stack& stack) {
        ...
        CUW3_ASSERT(allocations.size() == stack.limit, "invalid count of allocations");
        for (uint i = 1; i < allocations.size(); i++) {
            CUW3_ASSERT(allocations[i - 1] < allocations[i], "invalid order of allocated links.");
        }
        for (uint i = 0; i < allocations.size(); i++) {
            CUW3_ASSERT(allocations[i] == i, "invalid contents of allocated links.");
        }
    }
}

void test_atomic_stack_st(uint count) {
    using namespace atomic_stack_tests;
...
TEST(Atomic, AtomicStackSt) {
    test_atomic_stack_st(10000);
}
...
void test_atomic_stack_mt(uint count, uint threads) {
    ...
    CUW3_ASSERT(sum == stack.limit, "invalid count of allocations");
...
TEST(Atomic, AtomicStackMt) {
    for (int i = 0; i < 16; i++) {
        test_atomic_stack_mt(10000, 8);
    }
}
...
namespace atomic_list_tests {
    ...
    struct ListNodeOps {
        void set_next(ListLinkType node, ListLinkType next) {
            CUW3_ASSERT(node < num_nodes, "invalid node id received");
...
void test_atomic_list_st(uint num_nodes) {
    using namespace atomic_list_tests;
    ...
    for (uint i = 0; i < list.num_nodes; i++) {
        ...
        CUW3_ASSERT(node != list.null_link, "invariant violation: list was empty");
        CUW3_ASSERT(node == list.num_nodes - 1 - i, "invariant violation: we popped the wrong node");
    }
    CUW3_ASSERT(list.empty(), "invariant violation: list was not empty");
    ...
    for (auto i = rev_even_iter(list.num_nodes); i > 0; i -= 2) {
        auto node = list.pop(0);
        CUW3_ASSERT(node != list.null_link, "invariant violation: list was empty");
        CUW3_ASSERT(node == i - 1, "invariant violation: we popped the wrong node");
    }
    CUW3_ASSERT(list.empty(), "invariant violation: list was not empty");
    ...
    for (auto i = rev_odd_iter(list.num_nodes); i > 0; i -= 2) {
        auto node = list.pop(0);
        CUW3_ASSERT(node != list.null_link, "invariant violation: list was empty");
        CUW3_ASSERT(node == i - 1, "invariant violation: list was not empty");
    }
    CUW3_ASSERT(list.empty(), "invariant violation: list was not empty");
...
TEST(Atomic, AtomicListSt) {
    test_atomic_list_st(10000);
    test_atomic_list_st(10001);
}
    ...
    auto barrier_structure_test = [&]() {
        auto check_list = [&] (List& list, uint32 thread_id) {
            auto check = [&] (ListLinkType id, ListDataNode& node) {
                ...
                CUW3_ASSERT(!visited[id], "already visited");
                CUW3_ASSERT(node.load() == label, "invalid list label");
                CUW3_ASSERT(id != node.next, "cycle detected");
                visited[id] = true;
            };
            list.traverse(check);
        };
        ...
        for (uint thread_id = 0; thread_id < num_threads; thread_id++) {
            ...
            for (auto node : context.popped) {
                CUW3_ASSERT(!visited[node], "node has been popped and pushed at the same time");

                visited[node] = true;

                auto& node_ref = list.get(node);
                auto thread_label = ListNodeLabel{(uint32)node, (uint32)thread_id};
                CUW3_ASSERT(node_ref.data == thread_label, "invalid label set on popped node");
            }
        }
        for (auto v : visited) {
            CUW3_ASSERT(v, "not all nodes were pushed");
        }
    };
...
TEST(Atomic, AtomicListMt) {
    for (int i = 0; i < 16; i++) {
        test_atomic_list_mt(50000, 1, 16, 200000);
    }
}
...
namespace atomic_push_snatch_tests {
    ...
    struct ListPart {
        ...
        void push(ListNode* node) {
            CUW3_ASSERT(valid_single_node(node), "node must be single node");
    ...
    struct List : ListPart, ListTraits {
        ...
        uint32 get_node_id(ListNode* node) const {
            CUW3_ASSERT(node, "node must not be nullptr");
            CUW3_ASSERT((uintptr)node >= (uintptr)nodes.get(), "invalid node given");

            uint id = node - nodes.get();
            CUW3_ASSERT(id < num_nodes, "node id is invalid");
...
void test_atomic_push_snatch_list_st(uint num_nodes) {
    using namespace atomic_push_snatch_tests;
    ...
    }
    {
        ...
        CUW3_ASSERT(traversed.size() == list.num_nodes, "invalid amount of nodes");
        for (uint i = 1; i < traversed.size(); i++) {
            auto prev = list.get_node_id(traversed[i - 1]);
            auto curr = list.get_node_id(traversed[  i  ]);
            CUW3_ASSERT(prev > curr, "invalid list structure");
        }
    }
...
TEST(Atomic, AtomicPushSnatchListSt) {
    test_atomic_push_snatch_list_st(1000);
}
...
void test_atomic_push_snatch_structure_st() {
    ...
    CUW3_ASSERT(nodes == expected_nodes, "not exactly");
    CUW3_ASSERT(tails == expected_tails, "not exactly");
}

TEST(Atomic, AtomicPushSnatchStructureSt) {
    test_atomic_push_snatch_structure_st();
}
    ...
    auto barrier_structure_test = [&] () {
        auto check_list_part = [&] (ListPart list_part, uint32 thread_id) {
            auto check = [&] (ListNode* node) {
                ...
                CUW3_ASSERT(!visited[node_id], "already visited");
                CUW3_ASSERT(node->load() == label, "invalid list label");
                CUW3_ASSERT(node != node->next, "cycle detected");
        ...
        for (auto v : visited) {
            CUW3_ASSERT(v, "not all nodes were visited");
        }
    };
...
TEST(Atomic, AtomicPushSnatchListMt) {
    for (int i = 0; i < 16; i++) {
        test_atomic_push_snatch_list_mt(100000, 16, 32, 200000);
    }
}